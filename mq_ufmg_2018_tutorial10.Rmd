...
Até agora trabalhamos apenas com infomações em páginas em HTML. Dada as marcações de um texto em HTML, conseguíamos com poucas funções importar um documento, identificar sua estrutura e extrair infomações com precisão de dentro dele. O pacote _rvest_ contém tais funções é suficiente para a tarefa.
...
A principal função do pacote _pdftools_ é _pdf\_text_. Esta função recebe como argumento o caminho de um arquivo em .pdf, seja em uma pasta de seu computador ou na internet. Vamos utilizar como exemplos o documento com a proposição em inteiro teor da última PEC apresentada na data em que o tutorial foi elaborado (24/07/2018) e, a seguir, a tese de doutorado do autor principal deste tutorial, Leonardo Barone. Veja o uso da função é simples:

```{r}
url_ultima_pec <- "http://www.camara.gov.br/proposicoesWeb/prop_mostrarintegra;jsessionid=CA2368955586C1D76CDAD5CF042D0DE2.proposicoesWebExterno2?codteor=1677577&filename=PEC+437/2018"
...
Ao convertermos o arquivo em .pdf em um vetor de texto no R, cada página é alocada em uma posição do vetor. A proposição da PEC tem 17 páginas. A tese, 178.
...
for (u in urls_pecs) {
  ...
  link_inteiro_teor <- pagina %>%
    html_nodes(xpath = "//span[@class = 'naoVisivelNaImpressao']/a[@class='rightIconified iconDetalhe linkDownloadTeor']") %>%
    html_attr(name = 'href')
    
  link_inteiro_teor <- paste0("http://www.camara.gov.br/proposicoesWeb/",
           link_inteiro_teor)
  
  texto <- pdf_text(link_inteiro_teor)
...
```{r}
num_pecs <- 1:10
df_pec   <- data_frame(num_pecs, texto_pecs)
```
