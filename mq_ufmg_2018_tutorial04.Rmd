...
for (link in dados_pesquisa$links){
  ...
  # Algumas páginas podem ter links quebrados. Por esta razão, fazemos:
  pagina <- try(expr = read_html(link), silent = T)  
  if(any(class(pagina) %in% "try-error")){
          next
  }
...
for (i in 1:5){
  ...
  # Como no caso anterior, algumas páginas podem ter links quebrados...
  pagina <- try(expr = read_html(link), silent = T)  
  if(any(class(pagina) %in% "try-error")){
          next
  }
...
for (link in dados_pesquisa$links){
  ...
  # Já viram isso antes, né? Então... páginas com links quebrados etc.
  pagina <- try(expr = read_html(link), silent = T)  
  if(any(class(pagina) %in% "try-error")){
          next
  }
