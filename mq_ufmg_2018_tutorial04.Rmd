# Tutorial 4 - Raspando notícias com XPath em 2 passos: links e conteúdos
...
AVISO: Esse código é bastante semelhante ao código do [Tutorial 3](https://github.com/antrologos/mq_2018_WebScraping/blob/master/mq_ufmg_2018_tutorial03.Rmd). Mudam apenas os urls e o xpath. Você pode passar por ele repidamente se quiser.
...
# Neste exemplo, diferentemente do que fizemos nos Tutoriais 2 e 3, não precisamos "voltar um passo" para coletar o atributo "href", que contém o link. Você consegue entender o por quê disso?
...
# Perceba que "texto" é um vetor de tipo character, composto de vários elementos. Queremos concatenar todos os seus elementos num só, para que ele seja um vetor de tamannho (length) igual a 1, tal como são "titulo" e "datahora".
texto      <- paste(texto, collapse = " ")
```
...
```{r, echo = F}
dados_noticias <- data_frame()

for (link in dados_pesquisa$links){
  print(link)
  
  # esse passo vai dar erro em algumas paginas... falaremos disso adiante
...
# Banco que vai guardar títulos e links
dados_pesquisa <- data_frame()
...
# Banco que vai guardar o conteúdo das notícias
...
# Vejam:
read_html(url_inexistente)
```

A cláusula de tentativa é introduzida pela função `try`:

``` {r}
pagina <- try(read_html(url_inexistente))
```

Parece que nada mudou, né? Mas agora, quando o URL não existe, o código não é interrompido e o objto página, que deveria ser da classe "xml_document", passa a ser da classe "try-error"
...
O problema é que ao executarmos `try(read_html(url_inexistente))`, continuamos a receber o erro... e isso interrompe a execução... Gostaríamos que a tentativa de coletar o conteúdo da página fosse feita "em silêncio", isto é, sem a exibição da mensagem de erro, caso exista algum erro.

Isso pode ser feito adicionando o argumento `silent = TRUE` na nossa função `try`:

``` {r}
pagina <- try(read_html(url_inexistente), silent = TRUE)
```

Agora vejam, a classe da página continua sendo "try-error", mas a execução não foi interrompida:
...
Pronto! Vamos adicionar esse pedaço de código (sem as mensagens) ao nosso programa completo e tentar novamente. Tente compreender como o código funciona. O resultado final, no momento da elaboração do tutorial, é de 242 notícias (considerando a quantidade de links quebrados, tal como existentes no dia 23/07/2018 -- data da última revisão deste material).
...
for (link in dados_pesquisa$links){
  ...
  pagina <- try(read_html(link), silent = TRUE) # Aqui está nossa inserção
  
  # Apenas vamos tentar raspar conteúdos das páginas que não deram erro:
  if (!("try-error" %in% class(pagina))){
    node_titulo <- html_nodes(pagina, xpath = "//h1[@class = 'main_color main_title']")
    titulo <- html_text(node_titulo)
