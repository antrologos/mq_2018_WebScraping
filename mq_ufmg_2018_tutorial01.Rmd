# Tutorial 1 - Princípios básicos: rvest e coleta de tabelas
...
# Atividade inicial - Pesquisa de Proposições na ALESP (Assembléia Legislativa de do Estado de São Paulo)
...
Há 2 informações sobre as proposições: título (que traz o número do projeto e também a data) e autor.
...
Também podemos observar que a pesquisa retornou 84 páginas ("lastPage=84") e que a página atual (2) é a de número 1 ("currentPage=1") -- a página 1 da pesquisa é numerada como 0 nesta ferramenta de busca, mas o padrão muda de página para página.

Podemos ver que há muitas páginas de resultado para a palavra-chave que utilizamos. Nosso desafio é conseguir "passar" de forma eficiente por elas, ou seja, acessar os 84 links e "raspar" o seu conteúdo. Para isso, usaremos uma função essencial na programação, o "for loop".
...
Note que a função _html\_table_ tem outros dois argumentos: "header = TRUE" e "fill = TRUE". O primeiro argumento serve para indicarmos ao nosso "extrator de tabelas HTML" que a primeira linha da tabela deve ser considerada cabeçalho e, portanto, servirá de nome às colunas da tabela. O segundo argumento diz para que o R preencha (com espaços vazios) partes da tabela que podem estar incompletas (como, por exemplo, linhas com números desiguais de colunas etc.). Trata-se mais de uma precaução, devida ao fato de que o conteúdo da página pode conter erros ou assimetrias. Neste caso, isto é mesmo verdade. Experimente executar o mesmo comando sem o argumento "fill".

Observe, contudo, que a tabela coletada ainda contém problemas. Há colunas e linhas vazias. Isso é normal. Boa parte do trabalho de coleta de dados da internet consiste exatamente em trabalhar e melhorar a informação que raspamos. E é sobre esse ponto que versará boa parte dos outros tutoriais. Por ora, nos damos por satisfeitos.
...
lista_tabelas <- html_table(pagina, fill = TRUE)

tabela <- lista_tabelas
class(tabela) # Uma lista

tabela <- lista_tabelas[[1]]
class(tabela) # Um data.frame

```

'data.frames', como veremos adiante, são os objetos que usualmente conhecemos como "bancos de dados". Ou seja: trata-se de uma organização tabular dos dados, em linhas e colunas. Este é o tipo de resultado que desejamos. Falaremos mais sobre os data.frame adiante.
...
for (i in 0:4) {
  ...
  lista_tabelas <- html_table(pagina, header = TRUE, fill = T)
...
Apesar de que que a tabela contenha 100 linhas, trata-se, na realidade de 50 observações (5 páginas com 10 resultados). As linhas em branco estão duplicando o número de casos. São também apenas 2 variáveis (documento e autor) -- ao invés de 7. Há colunas que foram preenchidas automaticamente... As 2 variáveis são do tipo "character" contêm as informações corretas. As 6 observações apresentam o resultado adequado, o que nos dá uma boa dica que que tudo ocorreu bem até a última página capturada.
